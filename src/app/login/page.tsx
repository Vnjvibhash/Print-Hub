...
function LoginContent() {
  ...
  const handleQuickAdminLogin = async () => {
    ...
    try {
      ...
      console.error(err);
      setErrorText(err.message || "Admin authentication failed. Please use vnjvibhash@gmail.com.");
    } finally {
      ...
      <div className="glass-panel border-white/5 rounded-3xl p-6 sm:p-8 shadow-2xl w-full space-y-6">
        ...
        {!isSignUpMode && (
          <button
            ...
            className="w-full relative overflow-hidden py-3 rounded-xl bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-600 bg-[length:200%_100%] animate-[shimmer-gradient_3s_linear_infinite] hover:shadow-lg hover:shadow-purple-500/20 text-white text-xs font-bold flex items-center justify-center space-x-2.5 transition-all duration-300 disabled:opacity-60 cursor-pointer group"
          >
            {adminLoading ? (
        ...
        {!isSignUpMode && (
          <div className="bg-indigo-500/5 border border-indigo-500/10 p-3 rounded-xl text-zinc-500 dark:text-zinc-450 text-[11px] text-center">
            <span className="font-semibold block text-indigo-600 dark:text-indigo-400 mb-0.5">Admin Google Account</span>
            Sign in with <code className="font-bold">vnjvibhash@gmail.com</code> for admin access
          </div>
        )}
